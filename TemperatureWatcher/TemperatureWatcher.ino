...
void setup() {
  ...
  // Splash
  lcd.setCursor(3, 1); lcd.print("BMP180 Station");
  delay(1500);
  lcd.clear();

  // BMP180 init
  lcd.setCursor(0, 0); lcd.print("[1/2] Sensor...");
  Serial.println("Initializing BMP180...");
  if (!bmp.begin()) {
    lcd.setCursor(0, 1); lcd.print("FAIL: BMP180 missing");
    Serial.println("BMP180 not found!");
    while (true) { delay(1000); }
  }
  lcd.setCursor(0, 1); lcd.print("OK: BMP180 ready");
  Serial.println("BMP180 OK");
  delay(1000);

  // WiFi connect
  lcd.setCursor(0, 2); lcd.print("[2/2] WiFi...");
  ...
  while (ssid[0] != '\0' && WiFi.status() != WL_CONNECTED && attempts < 15) {
    ...
    if (status != lastStatus || attempts % 5 == 0) {
      ...
      lcd.setCursor(0, 3); lcd.print(wifiStatusToString(status));
      lcd.print(" ("); lcd.print(attempts); lcd.print("/15)");
      lastStatus = status;
    }
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    if (getLocalTime(&ti)) {
      char buf[20]; strftime(buf, sizeof(buf), "%d.%m.%Y %H:%M:%S", &ti);
    ...
    lcd2.clear();
    lcd2.setCursor(0, 0); lcd2.print("WiFi: "); lcd2.setCursor(6, 0); lcd2.print(lcd2SsidVal);
    lcd2.setCursor(0, 1); lcd2.print("IP: ");   lcd2.setCursor(4, 1); lcd2.print(lcd2IpVal);
  }
...
void loop() {
  ...
  if (millis() - lastLcdMs >= 500) {
    ...
    char buf[15];
    snprintf(buf, sizeof(buf), "%6.1f\xDFC  ", temp);        lcd.setCursor(6, 0); lcd.print(buf);
    snprintf(buf, sizeof(buf), "%7.1f hPa",    pressureHPa); lcd.setCursor(6, 1); lcd.print(buf);
    snprintf(buf, sizeof(buf), "%7.1f mmH",    pressureMmHg);lcd.setCursor(6, 2); lcd.print(buf);
    snprintf(buf, sizeof(buf), "%8.1f m  ",    altitude);    lcd.setCursor(5, 3); lcd.print(buf);
  }
  ...
  if (digitalRead(RESET_BTN) == LOW) {
    ...
    if (digitalRead(RESET_BTN) == LOW) {
      lcd.clear();
      lcd.setCursor(0, 0); lcd.print("Hold 2s to reset");
      lcd.setCursor(0, 1); lcd.print("flash...");
      uint32_t holdStart = millis();
      while (digitalRead(RESET_BTN) == LOW) {
        delay(10);
        if (millis() - holdStart >= 2000) {
          lcd.clear();
          lcd.setCursor(0, 0); lcd.print("Resetting flash...");
